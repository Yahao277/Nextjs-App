...
const SidebarSection = (sectionProp: SidebarSectionProp) => {
  return (
    <Link
      className={`my-2 flex w-full items-center justify-start p-2 pl-6 ${
        sectionProp.active ? activeStyles : inactiveStyles
      }}`}
      replace={true}
      href={sectionProp.href}
    >
      <span className="text-left">{sectionProp.icon}</span>
      <span className="mx-2 text-sm font-normal">{sectionProp.name}</span>
    </Link>
  );
};
